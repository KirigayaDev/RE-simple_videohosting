...
from .schemas import AccountTokenPayloadSchema, JwtTokens

from .coder import encode_token


def generate_tokens(user: User, start_datetime: datetime.datetime) -> JwtTokens:
    access_payload = AccountTokenPayloadSchema(sub=user.uuid,
    ...
    refresh_payload = AccountTokenPayloadSchema(sub=user.uuid,
